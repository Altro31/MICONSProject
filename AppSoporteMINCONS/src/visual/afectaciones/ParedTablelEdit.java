...
public class ParedTablelEdit extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public ParedTablelEdit(Window window, TableModel tablePadre, ArrayList<Pared> lista, ArrayList<Material> listaMateriales, int numero) {
		...
		this.numero=numero;
		this.lista=lista;
		setTitle("Editar Pared Afectada");
		setAlwaysOnTop(true);
		...
		if (btnAceptar == null) {
			...
			btnAceptar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (add) {
						Pared pared = lista.get(tablePadre.getRowCount()-1);
						pared.setIdentificador(textIdentificador.getText());
						pared.setMaterialPredominante((Construccion) Afectaciones.buscarMateriar(comboBoxMatPred.getSelectedItem().toString()));
						pared.setTipoDerrumbe(TipoDerrumbe.getValue(comboBoxTipoDerrumbe.getSelectedItem().toString()));
						pared.setEsParedCarga(chckbxParedCarga.isSelected());
						actualizar();
						dispose();
	...
	public void actualizar() {
		...
		for(Pared pared : lista) {
			tablePadre.addRow(new Object[] {
					""+count++,
					pared.getIdentificador(),
					pared.getMaterialPredominante().getNombre(),
					pared.getTipoDerrumbe().name(),
