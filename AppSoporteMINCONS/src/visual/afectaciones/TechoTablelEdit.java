...
public class TechoTablelEdit extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public TechoTablelEdit(Window window, TableModel tablePadre, ArrayList<Techo> lista, ArrayList<Material> listaMateriales, int numero) {
		...
		this.numero=numero;
		this.lista=lista;
		setTitle("Editar Techo Afectada");
		setAlwaysOnTop(true);
		...
		groupLayout.setVerticalGroup(
			groupLayout.createParallelGroup(Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup()
					...
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(getPanel(), GroupLayout.PREFERRED_SIZE, 127, GroupLayout.PREFERRED_SIZE)
		...
		if (btnAceptar == null) {
			...
			btnAceptar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (add) {
						Techo Techo = lista.get(tablePadre.getRowCount()-1);
						Techo.setIdentificador(textIdentificador.getText());
						Techo.setMaterialPredominante(((Construccion) Afectaciones.buscarMateriar(comboBoxMatPred.getSelectedItem().toString())));
						Techo.setTipoDerrumbe(TipoDerrumbe.getValue(comboBoxTipoDerrumbe.getSelectedItem().toString()));
						actualizar();
						dispose();
	...
	public void actualizar() {
		...
		for(Techo Techo : lista) {
			tablePadre.addRow(new Object[] {
					""+count++,
					Techo.getIdentificador(),
					Techo.getMaterialPredominante().getNombre(),
